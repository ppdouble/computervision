...
class ResNet(object):
  ...
  def _build_train_op(self):
    ...
    grads = tf.gradients(ys=self.cost, xs=trainable_variables)
  ...
  def _bottleneck_residual(self, x, in_filter, out_filter, stride,
    ...
    tf.compat.v1.logging.info('image after unit %s', x.get_shape())
    return x
  ...
  def _batch_norm(self, name, x):
    with tf.compat.v1.variable_scope(name):
      ...
      if self.mode == 'train':
        ...
        # 获取训练中积累的batch均值、标准差
        mean = tf.compat.v1.get_variable('moving_mean', 
                               params_shape, tf.float32,
                               initializer=tf.compat.v1.constant_initializer(0.0, tf.float32),
                               trainable=False)
        variance = tf.compat.v1.get_variable('moving_variance', 
                                   params_shape, tf.float32,
                                   initializer=tf.compat.v1.constant_initializer(1.0, tf.float32),
                                   trainable=False)
        # 添加到直方图总结
  ...
  def _conv(self, name, x, filter_size, in_filters, out_filters, strides):
    with tf.compat.v1.variable_scope(name):
      ...
      kernel = tf.compat.v1.get_variable(
              ...
              tf.float32, 
              initializer=tf.compat.v1.random_normal_initializer(stddev=np.sqrt(2.0/n)))
      # 计算卷积
